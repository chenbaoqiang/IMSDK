...
public class SdkAlarm {
    ...
    public synchronized void set(String json) {
        try {
            JSONObject jsonObj = new JSONObject(json);
            String alarmName = jsonObj.getString("name");
            long delayMs = jsonObj.getLong("delay");
    ...
    public synchronized  void setRepeating(String json){
        try {
            JSONObject jsonObj = new JSONObject(json);
            String alarmName = jsonObj.getString("name");
            long intervalMs = jsonObj.getLong("interval");
            setRepeating(alarmName, intervalMs);
        }catch (Exception ex){
            Log.e(TAG, ex.toString());
        }
    }

    public synchronized void setRepeating(String name, long interval){
        set(name, interval, true);
    }
    ...
    private void set(String alarmName, long nextDelay, boolean repeat){
        Log.d(TAG, String.format("set alram %s %s %s",alarmName, nextDelay,repeat));
            ...
            BroadcastReceiver receiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    try {
                        ...
                        if(lastIntent == null) {
                            Log.d(TAG, String.format("unknow alarm:%s", intent.getAction()));
                            return;
                        }
                        ...
                        Log.d(TAG, String.format("receive Alarm:%s,action:%s, delay:%dms(delta:%d)",
                                name, intent.getAction(), lastIntent.delay, delta ));
                        if(state == null){
                            Log.e(TAG, "init state not completed!");
                        }else {
                            RcsApi.onbell(state, name, 1000, null);
                        }
